...
export function Select({ label, placeholder = 'Select', options, value, onChange }: SelectProps) {
  ...
  const renderItem = ({ item }: ListRenderItemInfo<SelectOption>) => (
    ...
    >
      <Text className={`text-base ${item.value === value ? 'font-semibold text-emerald-600' : 'text-gray-900'}`}>
        {item.label}
      </Text>
      {item.value === value && (
  ...
  return (
    <View>
      ...
      >
        <Text className={selected ? 'text-base text-gray-900' : 'text-base text-gray-300'}>
      ...
      <Modal visible={open} transparent animationType="slide">
        ...
        <View className="bg-white rounded-t-3xl max-h-[50%]">
          <View className="px-5 py-4 border-b border-gray-100 flex-row items-center justify-between">
            <Text className="text-base font-bold text-gray-900">{label || 'Select'}</Text>
