    ...
    >
      ...
      {!compact && (
        <Pressable
          ...
          className="flex-row items-center justify-between px-4 py-2 border-t border-gray-50 dark:border-gray-800"
        >
          <Text className="text-xs font-medium text-gray-500 dark:text-gray-400">Score Breakdown</Text>
          <Ionicons name={expanded ? 'chevron-up' : 'chevron-down'} size={14} color={isDark ? '#6b7280' : '#9ca3af'} />
        </Pressable>
      )}
      ...
      {expanded && !compact && (
        <View className="px-4 pb-3">
          ...
          {/* Discipline */}
          <Text className="text-[10px] font-semibold text-gray-400 dark:text-gray-500 uppercase tracking-wider mb-1 mt-2">
            Discipline (50%)
          </Text>
      ...
      {displayActions.length > 0 && (
        <View className="border-t border-gray-50 dark:border-gray-800 px-4 py-2">
          {compact && (
            <Text className="text-[10px] font-semibold text-gray-400 dark:text-gray-500 uppercase tracking-wider mb-1.5">
              Improve your score
            </Text>
          )}
          {displayActions.map((action) => (
            <View key={action.id} className="flex-row items-center gap-2 py-1">
              ...
              <Text className="text-[10px] text-gray-400 dark:text-gray-500">{action.target}</Text>
            </View>
          ))}
        </View>
      )}
